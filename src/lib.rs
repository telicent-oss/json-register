...
impl Register {
    pub async fn new(
        ...
        let db = Db::new(connection_string, table_name, id_column, jsonb_column, pool_size)
            .await
    ...
    pub async fn register_object(&self, value: &Value) -> Result<i64, JsonRegisterError> {
        ...
        let id = self.db.register_object(&canonical).await.map_err(JsonRegisterError::DbError)?;
    ...
    pub async fn register_batch_objects(&self, values: &[Value]) -> Result<Vec<i64>, JsonRegisterError> {
        ...
        let ids = self.db.register_batch_objects(&canonicals).await.map_err(JsonRegisterError::DbError)?;
...
impl PyJsonRegister {
    ...
    fn new(
        ...
        if database_name.is_empty() {
            return Err(JsonRegisterError::Configuration("database_name cannot be empty".into()).into());
        }
        ...
        let inner = rt.block_on(async {
            Register::new(&connection_string, table_name, id_column, jsonb_column, pool_size, lru_cache_size).await
        })?;
    ...
    fn register_object(&self, json_obj: &Bound<'_, PyAny>) -> PyResult<i64> {
        let value: Value = pythonize::depythonize(json_obj).map_err(|e| JsonRegisterError::SerializationError(e.to_string()))?;
        self.rt.block_on(self.inner.register_object(&value)).map_err(Into::into)
    }
    ...
    fn register_batch_objects(&self, json_objects: &Bound<'_, PyList>) -> PyResult<Vec<i64>> {
        ...
        for obj in json_objects {
             let value: Value = pythonize::depythonize(&obj).map_err(|e| JsonRegisterError::SerializationError(e.to_string()))?;
             values.push(value);
        }
        self.rt.block_on(self.inner.register_batch_objects(&values)).map_err(Into::into)
    }
}
